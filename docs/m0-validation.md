...
- [x] Private-use Powerline and Nerd Font glyphs use the pinned, bundled `Symbols Nerd Font Mono` v3.5.1 face instead of ambiguous system PUA fallback.
- [x] SGR 2 dim text blends foreground and underline colors toward the cell background while SGR 22 restores normal intensity.
- [x] Settings use a full-window, focus-owning overlay that blocks PTY input while the shell continues running and restores terminal focus when closed.
...
The Linux CI smoke test requires the `libxkbcommon-x11-0` runtime package in addition to the xkbcommon development headers. A missing custom terminal font must show a fallback notice and render with the bundled font rather than silently selecting a proportional system face. The Nerd Fonts v3.5.1 archive and bundled font checksums are recorded in `crates/matcha-ui/assets/fonts/NERD-FONTS-PROVENANCE.md`.
